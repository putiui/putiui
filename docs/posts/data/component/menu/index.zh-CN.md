...
|router|boolean|false|设置子组件`MenuItem`以vue-router模式跳转|
|router-replace|boolean|false|设置router以replace方式调用，仅在`router`值为真时有效|
...
| 名称  | 说明 |
| - | - |
| click(sign) |子组件`MneuItem`单击时触发，传递该`MenuItem`的`sign`值|
...
|router|boolean|`Menu.router`|设置以vue-router模式跳转，默认值取父组件`Menu`的`router`属性值|
|to|string||设置跳转链接，`router`值为真时，作用同`router-link`的`to`属性，否则作用同`a`标签的`href`属性|
|router-replace|boolean|`Menu.router-replace`|设置router以replace方式调用，仅在`router`值为真时有效|
|target|string||设置跳转目标，作用同`a`标签的`target`属性，仅当`router`值不为真时有效|
...
|router|boolean|`SubMenu.router`|设置以vue-router模式跳转，默认值取父组件`SubMenu`的`router`属性值，此值为`true`时，请务必设置`to`属性|
|router-replace|boolean|`SubMenu.router-replace`|设置router以replace方式调用，仅在`router`值为真时有效|
