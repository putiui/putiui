...
<td>string</td>
<td></td>
<td>mini, tiny, small, medium, large, big, huge, massive</td>
<td>设置所有包含<code>size</code>选项的组件设置默认值</td>
</tr>
<tr>
<td>组件名称</td>
<td>object</td>
<td>组件默认配置</td>
<td></td>
<td>如要修改<code>pt-button</code>组件的默认配置，可使用<code>Vue.use(PutiUI,{ PtButton: {...}})</code>的方式进行配置，其他组件类似</td>
</tr>
</tbody>
</table>
<h3>按需引入时</h3>
...
<li>如果组件本身存在的配置项与PtUI全局的配置项名称一致时，优先获取组件本身的配置</li>
         ...
         </template>         <template slot="demo">                      </template>         <template slot="spec">                      </template>     </ComArticle> </template>  <script> /* eslint-disable */ import ComArticle from '../../../components/ComArticle';   export default {     name: 'GuideQuickstart',     data() {         return {             name: 'GuideQuickstart',             meta: {"category":"Guide","order":"1","title":"快速上手"},             demo:null         }     },     components:{         ComArticle,                       } } </script> 